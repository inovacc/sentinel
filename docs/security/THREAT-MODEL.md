...
| T1.3 | D | Attacker floods bootstrap port to exhaust connections / TLS handshakes | M | Bootstrap port now **closed once mTLS is established** (`shouldOpenBootstrap`); only open for initial pairing, `serve --renew-certs`, or the time-boxed `sentinel renew` window. Per-IP rate limit still pending. | `cmd/serve.go`, `cmd/renew.go` | `cmd/serve_renew_test.go` | 🟡 — lifecycle closed 2026-06-03; rate limit → Phase 3.2 |
