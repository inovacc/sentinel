...
| T2.3 | S | Compromised cert continues to work indefinitely | H | Short-lived certs (≤30d) + local revocation list checked on every mTLS handshake | `internal/ca/` (`CertValidity` 720h, `RenewThreshold` 240h, auto-renew), `pkg/transport/mtls.go` (`VerifyPeer`), `internal/fleet/registry.go` (revoked cols) — Phase 3.4 `docs/superpowers/plans/2026-06-04-crypto-hardening.md` | `internal/ca/validity_test.go`, `pkg/transport/mtls_revoke_test.go`, `internal/fleet/registry_revoke_test.go` | ✅ — mitigated Phase 3.4 (2026-06-04) |
...
| ID | STRIDE | Threat | Sev | Mitigation | Code | Test | Status |
|----|--------|--------|-----|------------|------|------|--------|
| T8.1 | I | Local attacker reads CA private key from disk | C | AES-256-GCM envelope encryption of CA key; DEK in OS keystore (DPAPI/Keychain/libsecret) or argon2id-wrapped for headless hosts; daemon fails closed on unseal failure | `internal/security/crypto/` (keystore + AES-GCM), `internal/ca/` (Sealer) — Phase 3.4 `docs/superpowers/plans/2026-06-04-crypto-hardening.md` | `internal/security/crypto/*_test.go`, `internal/ca/sealedkey_test.go` | ✅ — mitigated Phase 3.4 (2026-06-04) |
| T8.2 | T | Attacker swaps CA key with their own | C | GCM auth tag detects tampering; cert/key public-key match verified at load; file mode `0600`; HSM deferred to backlog | `internal/ca/` (public-key match at load), `internal/security/crypto/` (GCM tag) — Phase 3.4 `docs/superpowers/plans/2026-06-04-crypto-hardening.md` | `internal/ca/sealedkey_test.go`, `internal/security/crypto/*_test.go` | ✅ — mitigated Phase 3.4 (2026-06-04) |
...
| T8.4 | E | Old long-lived cert remains valid even after device is revoked | C | Local revocation: fleet `revoked`/`revoked_at`/`reason` columns; `VerifyPeer` hook rejects revoked peers on every mTLS handshake; live sessions closed on heartbeat; `sentinel revoke`/`unrevoke` CLI; distributed CRL/OCSP deferred to backlog | `internal/fleet/registry.go` (revoked cols), `pkg/transport/mtls.go` (`VerifyPeer`), `cmd/revoke.go` — Phase 3.4 `docs/superpowers/plans/2026-06-04-crypto-hardening.md` | `internal/fleet/registry_revoke_test.go`, `pkg/transport/mtls_revoke_test.go`, `cmd/revoke_test.go` | ✅ — mitigated Phase 3.4 (2026-06-04) |
